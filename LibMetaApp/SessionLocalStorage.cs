...
namespace LibMetaApp
{
    public class SessionLocalStorage : ISessionLocalStorage
    {
        ...
        public async Task SaveSessionAsync(SessionMarkerLiteModel set_session_marker)
        {
            ...
            await _local_store.SetItemAsync(SESSION_STORAGE_KEY_LOGIN, set_session_marker.Login);
            await _local_store.SetItemAsync(SESSION_STORAGE_KEY_LEVEL, set_session_marker.AccessLevelUser);
            await _local_store.SetItemAsync(SESSION_STORAGE_KEY_TOKEN, set_session_marker.Token);
        }
